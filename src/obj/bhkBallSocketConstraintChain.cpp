...
void bhkBallSocketConstraintChain::Read( istream& in, list<unsigned int> & link_stack, const NifInfo & info ) {
	...
	if ( (numLinks >= 1) ) {
		...
		for (unsigned int i2 = 0; i2 < links.size(); i2++) {
	...
	if ( (numLinks2 >= 1) ) {
		...
		for (unsigned int i2 = 0; i2 < links2.size(); i2++) {
			NifStream( block_num, in, info );
			link_stack.push_back( block_num );
		};
	};
...
void bhkBallSocketConstraintChain::Write( ostream& out, const map<NiObjectRef,unsigned int> & link_map, list<NiObject *> & missing_link_stack, const NifInfo & info ) const {
	...
	if ( (numLinks >= 1) ) {
		for (unsigned int i2 = 0; i2 < links.size(); i2++) {
			if ( info.version < VER_3_3_0_13 ) {
				WritePtr32( &(*links[i2]), out );
			} else {
				if ( links[i2] != NULL ) {
					map<NiObjectRef,unsigned int>::const_iterator it = link_map.find( StaticCast<NiObject>(links[i2]) );
					if (it != link_map.end()) {
						NifStream( it->second, out, info );
						missing_link_stack.push_back( NULL );
					} else {
						NifStream( 0xFFFFFFFF, out, info );
						missing_link_stack.push_back( links[i2] );
					}
				} else {
					NifStream( 0xFFFFFFFF, out, info );
					missing_link_stack.push_back( NULL );
	...
	if ( (numLinks2 >= 1) ) {
		for (unsigned int i2 = 0; i2 < links2.size(); i2++) {
			if ( info.version < VER_3_3_0_13 ) {
				WritePtr32( &(*links2[i2]), out );
			} else {
				if ( links2[i2] != NULL ) {
					map<NiObjectRef,unsigned int>::const_iterator it = link_map.find( StaticCast<NiObject>(links2[i2]) );
					if (it != link_map.end()) {
						NifStream( it->second, out, info );
						missing_link_stack.push_back( NULL );
					} else {
						NifStream( 0xFFFFFFFF, out, info );
						missing_link_stack.push_back( links2[i2] );
					}
				} else {
					NifStream( 0xFFFFFFFF, out, info );
					missing_link_stack.push_back( NULL );
...
void bhkBallSocketConstraintChain::FixLinks( const map<unsigned int,NiObjectRef> & objects, list<unsigned int> & link_stack, list<NiObjectRef> & missing_link_stack, const NifInfo & info ) {
	...
	if ( (numLinks >= 1) ) {
		for (unsigned int i2 = 0; i2 < links.size(); i2++) {
			links[i2] = FixLink<NiObject>( objects, link_stack, missing_link_stack, info );
		};
	};
	if ( (numLinks2 >= 1) ) {
		for (unsigned int i2 = 0; i2 < links2.size(); i2++) {
			links2[i2] = FixLink<NiObject>( objects, link_stack, missing_link_stack, info );
		};
	};
